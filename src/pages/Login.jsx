    ...
    <main className="auth-page">
      ...
      <section className="auth-card botanical-card">
        ...
        <form className="stack-form" onSubmit={submit}>
          ...
          <label>Mật khẩu *<span className="password-field"><input type={show ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} required /><button type="button" onClick={() => setShow((value) => !value)}>{show ? <EyeOff size={17} /> : <Eye size={17} />}</button></span></label>
          <button className="button button--wide button--large" type="submit" disabled={busy}><LogIn size={18} /> {busy ? 'Đang đăng nhập...' : 'Đăng nhập'}</button>
        </form>
